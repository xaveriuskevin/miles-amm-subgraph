...
let BLACKLISTED_PAIRS: string[] = [
  '0x6a78e84fa0edad4d99eb90edc041cdbf85925961', // AIDOGE/WMNT
]
...
export function handleTransfer(event: Transfer): void {
  ...
  // case where direct send first on MNT withdrawls
...
export function handleSync(event: Sync): void {
  ...
  uniswap.totalLiquidityMNT = uniswap.totalLiquidityMNT.minus(pair.trackedReserveMNT as BigDecimal)
  ...
  // update MNT price now that reserves could have changed
  let bundle = Bundle.load('1')
  bundle.mntPrice = getEthPriceInUSD()
  bundle.save()

  token0.derivedMNT = findEthPerToken(token0 as Token)
  token1.derivedMNT = findEthPerToken(token1 as Token)
  token0.save()
  token1.save()

  // get tracked liquidity - will be 0 if neither is in whitelist
  let trackedLiquidityMNT: BigDecimal
  if (bundle.mntPrice.notEqual(ZERO_BD)) {
    trackedLiquidityMNT = getTrackedLiquidityUSD(pair.reserve0, token0 as Token, pair.reserve1, token1 as Token).div(
      bundle.mntPrice
    )
  } else {
    trackedLiquidityMNT = ZERO_BD
  }

  // use derived amounts within pair
  pair.trackedReserveMNT = trackedLiquidityMNT
  pair.reserveMNT = pair.reserve0
    .times(token0.derivedMNT as BigDecimal)
    .plus(pair.reserve1.times(token1.derivedMNT as BigDecimal))
  pair.reserveUSD = pair.reserveMNT.times(bundle.mntPrice)

  // use tracked amounts globally
  uniswap.totalLiquidityMNT = uniswap.totalLiquidityMNT.plus(trackedLiquidityMNT)
  uniswap.totalLiquidityUSD = uniswap.totalLiquidityMNT.times(bundle.mntPrice)
...
export function handleMint(event: Mint): void {
  ...
  // get new amounts of USD and MNT for tracking
  let bundle = Bundle.load('1')
  let amountTotalUSD = token1.derivedMNT
    .times(token1Amount)
    .plus(token0.derivedMNT.times(token0Amount))
    .times(bundle.mntPrice)
...
export function handleSwap(event: Swap): void {
  ...
  // get total amounts of derived USD and MNT for tracking
  let derivedAmountMNT = token1.derivedMNT
    .times(amount1Total)
    .plus(token0.derivedMNT.times(amount0Total))
    .div(BigDecimal.fromString('2'))
  let derivedAmountUSD = derivedAmountMNT.times(bundle.mntPrice)

  // TODO: AMOUNT TOO LOW
  // if(derivedAmountMNT < BigDecimal.fromString('0.000002') ) {
  //   // DO NOT MANAGE TOO LOW SWAP
  //   return
  // }

  // get total swap fee of derived USD and MNT for tracking
  let feeAmountMNT = token1.derivedMNT.times(amount1In).times(pair.token1FeePercent)
    .plus(token0.derivedMNT.times(amount0In).times(pair.token0FeePercent)).div(BigDecimal.fromString('100'))
  let feeAmountUSD = feeAmountMNT.times(bundle.mntPrice)

  // only accounts for volume through white listed tokens
  let trackedAmountUSD = getTrackedVolumeUSD(amount0Total, token0 as Token, amount1Total, token1 as Token, pair as Pair)

  let trackedAmountMNT: BigDecimal
  if (bundle.mntPrice.equals(ZERO_BD)) {
    trackedAmountMNT = ZERO_BD
  } else {
    trackedAmountMNT = trackedAmountUSD.div(bundle.mntPrice)
  }
  ...
  uniswap.totalVolumeMNT = uniswap.totalVolumeMNT.plus(trackedAmountMNT)
  ...
  uniswap.totalFeeMNT = uniswap.totalFeeMNT.plus(feeAmountMNT)
  uniswap.txCount = uniswap.txCount.plus(ONE_BI)
  ...
  uniswapDayData.dailyVolumeMNT = uniswapDayData.dailyVolumeMNT.plus(trackedAmountMNT)
  uniswapDayData.dailyFeeMNT = uniswapDayData.dailyFeeMNT.plus(feeAmountMNT)
  ...
  token0DayData.dailyVolumeMNT = token0DayData.dailyVolumeMNT.plus(amount0Total.times(token0.derivedMNT as BigDecimal))
  token0DayData.dailyVolumeUSD = token0DayData.dailyVolumeUSD.plus(
    amount0Total.times(token0.derivedMNT as BigDecimal).times(bundle.mntPrice)
  )
  token0DayData.save()

  // swap specific updating
  token1DayData.dailyVolumeToken = token1DayData.dailyVolumeToken.plus(amount1Total)
  token1DayData.dailyVolumeMNT = token1DayData.dailyVolumeMNT.plus(amount1Total.times(token1.derivedMNT as BigDecimal))
  token1DayData.dailyVolumeUSD = token1DayData.dailyVolumeUSD.plus(
    amount1Total.times(token1.derivedMNT as BigDecimal).times(bundle.mntPrice)
  )
  token1DayData.save()
}
